...
public class AutoBalance extends CommandBase {
    ...
    public void execute() {
        ...
        //things commented out allow for roll conterol, but there are a couple issues that need to be ironed out
        if((/*Math.abs(drivetrain.getRoll2d().getDegrees()) > AutoBalanceConstants.OPTIMAL_ROLL ||*/ Math.abs(drivetrain.getPitch2d().getDegrees()) > AutoBalanceConstants.OPTIMAL_PITCH) && pitchDelta < AutoBalanceConstants.THRESHOLD_ANGLE) { //maybe add check for theoretical color sensor?
           drivetrain.drive(new ChassisSpeeds(drivetrain.percentOutputToMetersPerSecond(pid.calculate(drivetrain.getPitch2d().getDegrees(), 0)),
                                              drivetrain.percentOutputToMetersPerSecond(0), //-pid.calculate(drivetrain.getRoll2d().getDegrees(), 0)),  
